...
${logPrefix}: Please specify options for this machine`);
  ...
  const events = isUndefined(options.events) ? wrapEmitter((0, import_mitt.default)()) : isEventEmitter(options.events) && wrapEmitter(options.events);
  if (!events) {
    throw new TypeError(`
${logPrefix}: Invalid event-emitter specified in options`);
  ...
  const internalEvents = wrapEmitter((0, import_mitt.default)());
...
Check your performTransitions() config.`;
  ...
  const switchMethods = Object.keys(INTERNAL_EVENTS).reduce((obj, methodName) => ({
    ...obj,
    [methodName]: (cb) => {
      const err = argTypeError2({ cb: isFunction })(methodName)(cb);
      if (err) {
        throw new TypeError(err);
      }
  ...
  function Emit(eventName, ...curriedArgs) {
    const err = argTypeError2({ eventName: isString })("Emit")(eventName);
    if (err) {
      throw new TypeError(err);
    }
  ...
  function Enter(state, ...curriedArgs) {
    const err = argTypeError2({ state: isString })("Enter")(state);
    if (err) {
      throw new TypeError(err);
    }
  ...
  function InState(...args) {
    const err = argTypeError2({ state: [isString, isPojo] })("InState")(args[0]);
    if (err) {
      throw new TypeError(err);
    }
    return isPojo(args[0]) ? _InStateObject(...args) : _InState(...args);
