...
(function (global, factory) {
  ...
  const typeErrorStringIfFnReturnsFalse = (argName, argTypeFn, arg) => {
    return argTypeFn(arg) ? undefined : (argTypeFn.displayName || argTypeFn.name) + `(${argName}) did not return true`;
  };
  const typeErrorStringIfTypeOfFails = (argName, argType, arg) => {
    return typeof arg === argType ? undefined : `Argument "${argName}" should be a ${argType}`;
  };
  ...
  function Statebot(name, options) {
    ...
    const {
      ...
      paused,
      Pausable
    } = Pausables(false, () => _console.warn(`${logPrefix}: Ignoring callback, paused`));
    ...
    function transitionNoOp(message) {
      ...
      const inState = currentState();
      const prevRoute = `${isUndefined(lastState) ? '[undefined]' : lastState}->${inState}`;
      ...
      if (!availableStates.length) {
        ...
        _console.info(`${logPrefix}: ${message}\n` + `  > Previous transition: "${prevRoute}"\n` + `  > From "${inState}", valid states are: [${availableStates.map(state => `"${state}"`).join(', ')}]`);
      }
    }
    function inspect() {
      return {
